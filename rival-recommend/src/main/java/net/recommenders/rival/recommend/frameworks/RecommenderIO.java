...
public final class RecommenderIO {
	 ...
	 *            the recommendations
	 * @param path
	 *            directory where fileName will be written (if not null)
	 * @param fileName
	 *            name of the file, if null recommendations will not be printed
	 * @param append
	 *            flag to decide if recommendations should be appended to file
	 * @param model
	 *            if not null, recommendations will be saved here
	 */
	public static void writeData(final long user, final List<Preference<Long, Long>> recommendations, final String path,
			final String fileName, final boolean append, final TemporalDataModelIF<Long, Long> model) {
		...
		try {
			...
			if ((path != null) && (fileName != null)) {
				out = new BufferedWriter(
						new OutputStreamWriter(new FileOutputStream(path + "/" + fileName, append), "UTF-8"));
			}
			for (Preference<Long, Long> recItem : recommendations) {
